...
<h1>	<homepage_body>8mu v1 Manual and Support</homepage_body></h1>

&nbsp;
* **[8mu product page](/8mu/)**
* **[8mu v2 Manual and Support](/8mu_v2_docs/)**
...
* **No MIDI in my DAW**: If the 8mu faders move in [The Editor](https://tomwhitwell.github.io/Smith-Kakehashi/), that shows that MIDI signals are being generated by the 8mu, and received by your computer. If they're not appearing in your DAW or other software, it's probably a config issue in that software, rather than 8mu.
...
* **Dead faders**: Dead faders - where a fader output is stuck on high, or low, or in the middle - can caused by soldering problems. To troubleshoot a dead channel...
  * First ensure you've installed [the latest firmware](https://github.com/TomWhitwell/Smith-Kakehashi/releases) as before 1.0.2 dead channels could be caused by firmware bug. 
  ...
  * **Then** you can be confident it might be hardware, so remove the USB cable, take off the back panel and carefully check your soldering
    ...
    * NB: You don't need to solder the clips in the middle of the fader - you need to fold them down
...
* **Scratchy faders** In a very few early units, the slots for the faders were too narrow. If your faders feel stiff or scratchy, [contact Thonk support](mailto:support@thonk.co.uk) to ask for help. 
* **8mu seems dead** with strange LEDs or no LEDs. If you've been experimenting or are checking a board for someone else, it could be the firmware isn't properly installed. Here's how to [troubleshoot an 8mu board](#troubleshooting-a-bare-board). 
...
Firmware: 
* May 2023: v1.0.2
    * Fixes a bug (thanks, Marcus) where TRS midi was only sending on Channel 1, ignoring the channel settings from the editor. Hardware midi should now send on the correct channel.
    * Fixes some bugs (Thanks, Tom) in the smoothing code on all channels, so faders should settle more confidently, and IMU channels are slightly smoother than they were previously.
    * Fixes a rare issue where some boards had stuck or dead faders.
